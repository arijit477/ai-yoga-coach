...
export function PostureCheckPanel({
  ...
  const issueCount = items.filter(
    (i) =>
      i.status === "warning" || i.status === "bad" || i.status === "critical",
  ).length;
    ...
    ? items
        ...
        .concat(
          items
            ...
            .slice(
              0,
              Math.max(0, 4 - items.filter((i) => i.status !== "good").length),
            ),
        )
    : items;
  ...
  return (
    ...
    >
      ...
      >
        <h3
          className={`${compact ? "text-[11px]" : "text-xs"} font-bold uppercase tracking-wider text-slate-500`}
        >
          Posture Check
        </h3>
